...
class OutputSection(QWidget):
    ...
    def show_adult_table(self):
        ...
        bmi_header.setFont(QFont("Arial", 11, QFont.Bold))
        condition_header = QLabel("Condition")
        condition_header.setFont(QFont("Arial", 11, QFont.Bold))

        table_layout.addWidget(bmi_header, 0, 0)
        table_layout.addWidget(condition_header, 0, 1)
        ...
        for row, (bmi_range, condition) in enumerate(data, start=1):
            table_layout.addWidget(QLabel(bmi_range), row, 0)
            table_layout.addWidget(QLabel(condition), row, 1)

        self.layout.addLayout(table_layout)

    def show_child_link(self):
        info = QLabel("For child's BMI interpretation, please click below:")
